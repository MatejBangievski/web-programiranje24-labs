...
public class DataHolder {
    ...
    public void init (){
        ...
        songs.add(new Song("first", "AfterHours", "R&B", 2020, albums.get(0), artists.stream().filter(a -> a.getId() == 0L).collect(Collectors.toList()))); //The Weeknd
        songs.add(new Song("second", "Rumble", "EDM", 2023, albums.get(1), artists.stream().filter(a -> a.getId() == 1L || a.getId() == 2L).collect(Collectors.toList()))); //FredAgain, Skrillex
        songs.add(new Song("third", "Homecoming", "Hip Hop", 2006, albums.get(2), artists.stream().filter(a -> a.getId() == 4L).collect(Collectors.toList()))); //Ye
        songs.add(new Song("fourth", "Fein", "Rage", 2024, albums.get(3), artists.stream().filter(a -> a.getId() == 3L).collect(Collectors.toList()))); //Travis Scott
        songs.add(new Song("fifth", "K-POP", "Rage", 2023, albums.get(3), artists.stream().filter(a -> a.getId() == 0L || a.getId() == 3L).collect(Collectors.toList()))); //Travis Scott, The Weeknd
